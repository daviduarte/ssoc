...
import os
import sys
sys.path.append("/media/denis/e234b4fb-e9ce-4141-a5cd-ef7dd1169bae/CamNuvem/pesquisa/extractI3d/pytorch-resnet3d/feature_extractors")
...
class TSN(nn.Module):
    ...
    def forward(self, input, no_reshape=False):
        if not no_reshape:
            ...
            print("Iput size -2")
            print(input.size()[-2:])
            print("Parametro passado para  base_model")
            print(input.view((-1, sample_len) + input.size()[-2:]).shape)
            print("Retorno")
            print(self.base_model(input.view((-1, sample_len) + input.size()[-2:])).shape)
            #                                                                      # 2 valores mais à direita                                                                                
        ...
        if self.get_global_pool:
            print("Entrou. ")
            print("NÃQO ERA PRA ENTRAR AQUI TTODO!!!!!")
            print("PORRRRRA")
            exit()
